 ...
 * @author Anton Kuzmin
 */
class Forms_Validate_Users_Groups_Name extends Zend_Validate_Abstract {
    ...
    protected $_messageTemplates = array(
        self::REPEAT => "L_GROUP_YET_EXISTS"
    );
    ...
    public function repeat($name) {
        ...
        if($this->_exclude) {
            $editServer = $Groups->get($this->_exclude);
            if($name == $editServer['name'])
                return false;
        }

        if($Groups->exists($this->_type, $this->_objectId, $name)) {
