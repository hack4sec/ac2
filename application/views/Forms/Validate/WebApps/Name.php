 ...
 * @author Anton Kuzmin
 */
class Forms_Validate_WebApps_Name extends Zend_Validate_Abstract {
    ...
    protected $_messageTemplates = array(
        self::REPEAT => "L_WEBAPP_YET_EXISTS"
    );
    ...
    public function repeat($title) {
        ...
        if($this->_exclude) {
            $editServer = $WebApps->get($this->_exclude);
            if($title == $editServer['name'])
                return false;
        }

        if($WebApps->exists($this->_domainId, $title)) {
